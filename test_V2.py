    ...
    with open(raw_path + '/loc_sorted'+ str(i)+'.json') as f:
        ...
        for obj in d:
            ...
            while processed == False:
                ...
                #print("current <360 min int" + str(obj_time)+ " " + str(divmod((obj_time-date_time_obj_start).total_seconds(), 60)[0]) + " " +str(date_time_obj_start))
                if date_time_obj_start > datetime.datetime.strptime("2020-04-11 00:00:00", "%Y-%m-%d %H:%M:%S"):
                    print("END")
                    processed = True
                    terminate = True
                    break
                if obj_time >= date_time_obj_start and divmod((obj_time-date_time_obj_start).total_seconds(), 60)[0] < 360:
                    #print("in interval :D ")
                    interval.append(obj)
                    #print(obj)
                    processed = True
            ...
            if terminate:
                break
    print(str(len(interval)) + " " +str(date_time_obj_start))
    if(len(interval)!=0):
        minVals = [100,100,100,100,100,100]
        maxVals = [0,0,0,0,0,0]
        meanVals = [[],[],[],[],[],[]]                
        ...
        data[str(date_time_obj_start)]  =  {'minimumVals' : str(minVals) , 'maximumVals' :  str(maxVals) ,'meanVals' : str(mean)}
        interval = []
        print("increment timeAAAAAAAAAAAAAAAAA ++ " + str(date_time_obj_start))
